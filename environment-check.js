...
function checkGoogleCloud() {
    ...
    if (fs.existsSync(credentialsPath)) {
        ...
        // Check if credentials are recent (less than 12 hours old)
        const stats = fs.statSync(credentialsPath);
        const ageInHours = (Date.now() - stats.mtime.getTime()) / (1000 * 60 * 60);
        checks.credentials.valid = ageInHours < 12;
        
        if (checks.credentials.valid) {
            log.success('Google Cloud credentials are valid ✓');
            checks.authenticated = true;
        } else {
            log.warning(`Credentials are ${Math.round(ageInHours)} hours old (>12h), may need refresh`);
        }
    } else {
